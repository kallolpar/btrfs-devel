		      ...
		      struct extent_buffer **cow_ret, u64 new_root_objectid)
{
	...
	write_extent_buffer(cow, fs_info->fsid, btrfs_header_fsid(),
			    BTRFS_FSID_SIZE);
		       ...
		       struct btrfs_key *progress)
{
	...
	for (i = start_slot; i <= end_slot; i++) {
		...
		cur = btrfs_find_tree_block(fs_info, blocknr);
		if (cur)
			     ...
			     int level, int slot, u64 objectid)
{
	...
	blocksize = fs_info->nodesize;
	eb = btrfs_find_tree_block(fs_info, search);
	if (eb) {
		free_extent_buffer(eb);
		return;
	}
				       ...
				       struct btrfs_path *path, int level)
{
	...
	if (slot > 0) {
		...
		eb = btrfs_find_tree_block(fs_info, block1);
		/*
	...
	if (slot + 1 < nritems) {
		...
		gen = btrfs_node_ptr_generation(parent, slot + 1);
		eb = btrfs_find_tree_block(fs_info, block2);
		if (eb && btrfs_buffer_uptodate(eb, gen, 1) != 0)
			block2 = 0;
		free_extent_buffer(eb);
	}
		       ...
		       struct btrfs_key *key, u64 time_seq)
{
	...
	tmp = btrfs_find_tree_block(fs_info, blocknr);
	if (tmp) {
			   ...
			   struct btrfs_path *path, int level)
{
	...
	write_extent_buffer(c, fs_info->fsid, btrfs_header_fsid(),
			    BTRFS_FSID_SIZE);
			       ...
			       struct btrfs_path *path, int level)
{
	...
	write_extent_buffer(split, fs_info->fsid,
			    btrfs_header_fsid(), BTRFS_FSID_SIZE);
	write_extent_buffer(split, fs_info->chunk_tree_uuid,
