				       ...
				       struct btrfs_fs_info *fs_info)
{
	...
	num_bytes <<= 1;
	num_bytes += btrfs_csum_bytes_to_leaves(fs_info, csum_bytes) * fs_info->nodesize;
			   ...
			   struct btrfs_root *root, unsigned long count)
{
	...
	if (fs_info->creating_free_space_tree)
		return 0;
...
again:
	...
	if (ret && flush != BTRFS_RESERVE_NO_FLUSH) {
		...
		if (!fs_info->log_root_recovering &&
		    need_do_async_reclaim(space_info, fs_info, used) &&
...
int btrfs_delalloc_reserve_metadata(struct inode *inode, u64 num_bytes)
{
	...
	to_reserve = btrfs_calc_trans_metadata_size(fs_info, nr_extents);
				     ...
				     u64 num_bytes)
{
	...
	delayed_refs = &trans->transaction->delayed_refs;
	spin_lock(&delayed_refs->lock);
	if (btrfs_qgroup_insert_dirty_extent(fs_info, delayed_refs, qrecord))
		kfree(qrecord);
	spin_unlock(&delayed_refs->lock);
			   ...
			   u64 size)
{
	 ...
	 */
	ret = update_space_info(fs_info, cache->flags, 0, 0,
				&cache->space_info);
	if (ret) {
		btrfs_remove_free_space_cache(cache);
		btrfs_put_block_group(cache);
